...
Bản update từ `1.0.0` lên `1.1.0` cập nhật trang device access: SSID, adapter và địa chỉ
IPv4 Wi-Fi được đọc lại khi mở/làm mới trang; mã QR được tạo lại từ URL hiện tại. Update runner
chạy từ bản sao tạm ngoài `Program Files`, sau đó áp dụng firewall rule LAN-scoped cho profile
Private/Public từ cây application
mới trước khi khởi động service.

Updater 1.2.0 có thể cập nhật bản cài 1.1.0: nó đọc version đang chạy từ `/health/`, tự dùng
...
chọn `ShopHoaThuan-Update-1.2.0.zip`, rồi kiểm tra `/health/` trả `version=1.2.0`. Không gỡ cài đặt
