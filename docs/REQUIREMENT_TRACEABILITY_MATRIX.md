...
|---|---|---|---|---|
| Một server ghi một data directory | `apps/core/operations.py`, `server.py` | process lease test, native server smoke, WinSW recovery test | PASS trên WSL và test service | production reboot/ACL còn cần xác minh |
...
| Service không migrate | `server.py`, WinSW XML, migration runner | runner tests, XML regression, native migration, service test | PASS | clean installer/reboot chưa chạy |
...
| Health | `apps/core/views.py`, `health_check.py` | healthy/maintenance/schema/db unavailable tests, native service recovery | PASS | clean install/reboot còn lại |
